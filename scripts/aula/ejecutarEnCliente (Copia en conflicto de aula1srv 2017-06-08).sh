...
usuario_lliurex() {
   ...
   for u in $(ls /home | grep -v lliurex); do
      echo sudo rm -rf /home/$u
   done   
}
[ "$(lliurex-version | grep -i server)" != "" ] && echo este script es para ejecutar en cliente solamente && exit 2
[ "$(hostname | grep srv)" != "" ] && echo este script es para ejecutar en cliente sólo && exit 2
