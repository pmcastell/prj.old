...
obtenerFichero() {
   ...
   if [ "$NUM_LINEAS" != "" ] && [ "$NUM_LINEAS" -gt $NUM_LINEAS_MIN ]; then
      TEMP2=$(tempfile)
      HOST_NAME=$(hostNAME)
      if $DEBUG; then HOST_NAME="aula2srv"; fi
      cat $TEMP | head  -$NUM_LINEAS | grep -E "(^$HOST_NAME|^GLOBAL)" > $TEMP2
      rm $TEMP
      mv $TEMP2 $TEMP
   else
      exit
...
borrarParametros() {
   HOST_NAME=$(hostNAME)
   for v in $(env); do 
      if [ "$(echo $v  | grep -E "(^$HOST_NAME|^GLOBAL)")" != "" ]; then
         unset $(echo $v | awk -F'=' '{print $1;}')
      fi
   done
}   
___procesarParametros() {
   borrarParametros
   TEMP=$(tempfile)
   HOST_NAME=$(hostNAME)
   if $DEBUG; then HOST_NAME="aula2srv"; fi
   ...
   while read LINEA; do 
      if [ "$LINEA" != "" -a "$(echo $LINEA | grep -E "(^$HOST_NAME|^GLOBAL)")" != "" ]; then
         VAR=$(echo $LINEA | awk -F'=' '{print $1;}')
         VALOR=$(echo $LINEA | awk -F'=' '{print $2;}')
         if [ "$(echo $VALOR | grep -E '^\$')" != "" -a "$(eval echo ${VALOR})" != "" ]; then
   ...
   #SSH
   export TUN_SSH=$(eval echo \$${HOST_NAME}_TUN_SSH); debug TUN_SSH: $TUN_SSH
   export TUN_SSH_IP=$(eval echo \$${HOST_NAME}_TUN_SSH_IP); debug TUN_SSH_IP: $TUN_SSH_IP
   export TUN_SSH_PORT=$(eval echo \$${HOST_NAME}_TUN_SSH_PORT); debug TUN_SSH_PORT: $TUN_SSH_PORT
   export TUN_SSH_DEV=$(eval echo \$${HOST_NAME}_TUN_SSH_DEV); debug TUN_SSH_DEV: $TUN_SSH_PORT
   ...
   export TUN_SSH_CMD=""
   for((i=1;i<=10;i++)); do 
      VAR_NAME="${HOST_NAME}_TUN_SSH_CMD${i}"
      export TUN_SSH_CMD="$TUN_SSH_CMD $(eval echo \$$VAR_NAME)"
   done
   export TUN_SSH_CMD=$(eval echo $TUN_SSH_CMD); debug TUN_SSH_CMD: $TUN_SSH_CMD
   #OVPN
   export TUN_OVP=$(eval echo \$${HOST_NAME}_TUN_OVP)
   export TUN_OVP_IP=$(eval echo \$${HOST_NAME}_TUN_OVP_IP)
   export TUN_OVP_PORT=$(eval echo \$${HOST_NAME}_TUN_OVP_PORT)
   export TUN_OVP_DEV=$(eval echo \$${HOST_NAME}_TUN_OVP_DEV)
...
procesarParametros() {
   ...
   export TUN_SSH_IP=$(eval echo \$${HOST_NAME}_TUN_SSH_IP); debug TUN_SSH_IP: $TUN_SSH_IP
   export TUN_SSH_PORT=$(eval echo \$${HOST_NAME}_TUN_SSH_PORT); debug TUN_SSH_PORT: $TUN_SSH_PORT
   export TUN_SSH_DEV=$(eval echo \$${HOST_NAME}_TUN_SSH_DEV); debug TUN_SSH_DEV: $TUN_SSH_DEV
   export TUN_SSH_DEV_IP="10.${TUN_SSH_DEV}.${TUN_SSH_DEV}.${TUN_SSH_DEV}"; debug TUN_SSH_DEV_IP: $TUN_SSH_DEV_IP
   export TUN_SSH_DEV_GW="10.${TUN_SSH_DEV}.${TUN_SSH_DEV}.1"; debug TUN_SSH_DEV_GW: $TUN_SSH_DEV_GW
   export TUN_SSH_CMD=$(eval echo \$${HOST_NAME}_TUN_SSH_CMD); debug TUN_SSH_CMD: $TUN_SSH_CMD
   #OVPN
   export TUN_OVP=$(eval echo \$${HOST_NAME}_TUN_OVP)
   export TUN_OVP_IP=$(eval echo \$${HOST_NAME}_TUN_OVP_IP)
   export TUN_OVP_PORT=$(eval echo \$${HOST_NAME}_TUN_OVP_PORT)
...
#CODIGO GUARDADO:
   ...
   while read LINEA; do 
      ...
      #$EXPORT; 
      if [ "$LINEA" != "" -a "$(echo $LINEA | grep $HOST_NAME | grep -v grep)" != "" ]; then export $LINEA; fi
