...
DIR_BASE=$(encuentraDir "hostinger")
if [ "$DIR_BASE" = "" ]; then echo No se han podido encontrar ficheros de índice; habla -n No se han podido encontrar ficheros de índice; exit 2; fi
INDICES="$DIR_BASE/indice4.html $DIR_BASE/indice5.html"
INDICES="$DIR_BASE/indice6.html $DIR_BASE/indice5.html"
...
#echo $PASSWD | openssl enc -e -aes-256-ctr -k "clave$(date -u +'%Y-%m-%d')" | base64 > /$DIR_BASE/indicep.html
echo $PASSWD > /$DIR_BASE/indicep.html
cd $DIR_BASE
./actualiza2.sh
TEMP=$(tempfile)
for INDICE in $INDICES; do
   wget -O - http://ubuin.hopto.org/$(basename $INDICE) 2>/dev/null | openssl enc -d -aes-256-ctr -k "clave$(date -u +'%Y-%m-%d')" > $TEMP
