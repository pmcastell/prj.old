...
uso() {
   cat<<EOF
   Uso: $0 <on|off>
   on: Pone dirección fija en este servidor
   off: Pone DHCP
EOF
   exit 1
}
...
[ "$(hostname)" = "aula1srv" ] && IP=1 || [ "$(hostname)" = "aula2srv" ] && IP=2 || echo "Error: hostnamme no encontrado" && exit 2
if [ "$1" = "on" ]; then
   killall dhclient &>/dev/null
   ROUTER=172.18.161.1
   ifconfig $INTERFAZ 172.18.161.25$IP/24
   route del default gw &>/dev/null
...
" > /etc/resolv.conf 
   ...
   ifconfig $INTERFAZ:1 10.10.10.1$IP/24
fi   
