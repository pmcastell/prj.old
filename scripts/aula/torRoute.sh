...
main() {   
    ...
    if [ "$2" = "" ]; then DEVS="9 11 12 15 16 17 88"; else DEVS=$2; fi
    ...
    while true; do
       for i in $DEVS; do
          if [ "$(ping -c 4 10.$i.$i.$i 2>&1 | grep '100% packet loss')" != "" ]; then
             while true; do
                ...
                if [ "$IP_TOR" != "" ] && [ "$(netstat -onatup | grep $SSHD | grep $IP_TOR)" = "" ]; then break; fi
                sleep 1;
             done
             
             DIR_IP_LOCAL="$(ip -4 a show $IFACE | grep inet | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}' | head -1)"
             ...
             if [ "$IP_TOR" != "" ] && [ "$(route -n | grep $IP_TOR)" = "" -a "$IP_TOR" != "$DIR_IP_LOCAL" ]; then # -a "$(wget -O - http://$IP_TOR/ 2>/dev/null | grep Tor)" != "" ]; then
                ...
                if $DEBUG; then sudo netstat -onatup | grep -i $SSHD >&2; fi
       ...
       while read IP_TOR; do 
          if [ "$(sudo netstat -onatup | grep $SSHD | grep $IP_TOR )" = "" ]; then
...
cambiarIface() {
    ...
    NUM_IFACES=$($IFACES | wc -l)
    if [ $NUM_IFACES -gt 1 ]; then
       for IFACE in $IFACES; do
          IP=$(ip -4 a show $IFACE | grep inet | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}' | head -1)
