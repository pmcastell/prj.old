...
configEth() {
    sudo firewall
    IFACE=$1; IP=$2; MASK=$3; GW=$4; DNS="$4"
...
nameservers() {
   ...
   done #sudo bash -c 'echo nameserver 172.16.1.9 > /etc/resolv.conf' #sudo bash -c 'echo nameserver 8.8.8.8 >> /etc/resolv.conf'
   route -n
   cat /etc/resolv.conf
   ping -c 2 $(head -1 /etc/resolv.conf | awk '{print $2;}')
   ping -c 2 $(head -2 /etc/resolv.conf | tail -1 | awk '{print $2;}')
...
casaBiblio() {
   ...
   if [ "$DONDE" = "9" ]; then 
      sudo eecho wpa_supplicant -B -i wlan20  -c /var/lib/wicd/configurations/5057a8671925 -Dwest & #/m/Mios/.../wicd/BIBLIO1 -Dwext &
   ...
   configEth $WIFACE "$RED.27" 24 "$RED.1" ###sudo ifconfig $IFACE:1 172.16.254.7/24
   ULTIMA_OVPN=$(cat /home/usuario/.bash_history | grep 'sudo openvpn --config' | grep -v grep  | tail -1)
   if [ "$ULTIMA_OVPN" = "" ]; then
...
cumples &
if [ -d /m/Mios/Instituto ]; then #eecho dropbox start -i
...
debug "estoy aquí"
sudo actualiza &
debug "estoy aquí después de actualiza"
