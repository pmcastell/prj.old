...
TARGET="/target/usr/sbin/password_users.sh"
echo "#!/bin/bash" > $TARGET

echo 'TMP=$(mktemp)' >> $TARGET
echo 'SHADOW="/etc/shadow"' >> $TARGET
echo 'PASSWD="/etc/passwd"' >> $TARGET
echo 'GROUP="/etc/group"' >> $TARGET

###echo 'awk -F: \\'{if ($1 == "root") $2="!*!"; print $1":"$2":"$3":"$4":"$5":"$6":"$7"::"}\\' $SHADOW > $TMP' >> $TARGET

###echo "cp $TMP $SHADOW" >> $TARGET

###echo "rm -f $TMP" >> $TARGET

echo "/usr/sbin/useradd --create-home --shell /bin/bash profesor" >> $TARGET

# Resetea la clave del usuario 'usuario'
echo 'echo "root:francisco" | /usr/sbin/chpasswd' >> $TARGET
echo 'echo "profesor:francisco" | /usr/sbin/chpasswd' >> $TARGET
echo 'echo "usuario:usuario" |  /usr/sbin/chpasswd' >> $TARGET

echo 'awk -F: '"'"'{ if($1=="sudo") $4=""; print $1":"$2":"$3":"$4}'"'"' $GROUP > $TMP' >> $TARGET

echo 'cp $TMP $GROUP' >> $TARGET
echo 'rm -f $TMP' >> $TARGET

chmod a+x /target/usr/sbin/password_users.sh
...
##############################################
# Configura los sources de apt
echo "deb http://archive.ubuntu.com/ubuntu/ precise main restricted" > /target/etc/apt/sources.list
echo "deb-src http://archive.ubuntu.com/ubuntu/ precise main restricted" >> /target/etc/apt/sources.list

echo "deb http://archive.ubuntu.com/ubuntu/ precise-updates main restricted" >> /target/etc/apt/sources.list
echo "deb-src http://archive.ubuntu.com/ubuntu/ precise-updates main restricted" >> /target/etc/apt/sources.list

echo "deb http://archive.ubuntu.com/ubuntu/ precise universe" >> /target/etc/apt/sources.list
echo "deb-src http://archive.ubuntu.com/ubuntu/ precise universe" >> /target/etc/apt/sources.list
echo "deb http://archive.ubuntu.com/ubuntu/ precise-updates universe" >> /target/etc/apt/sources.list
echo "deb-src http://archive.ubuntu.com/ubuntu/ precise-updates universe" >> /target/etc/apt/sources.list


echo "deb http://archive.ubuntu.com/ubuntu/ precise multiverse" >> /target/etc/apt/sources.list
echo "deb-src http://archive.ubuntu.com/ubuntu/ precise multiverse" >> /target/etc/apt/sources.list
echo "deb http://archive.ubuntu.com/ubuntu/ precise-updates multiverse" >> /target/etc/apt/sources.list
echo "deb-src http://archive.ubuntu.com/ubuntu/ precise-updates multiverse" >> /target/etc/apt/sources.list

echo "deb http://archive.ubuntu.com/ubuntu/ precise-backports main restricted universe multiverse" >> /target/etc/apt/sources.list
echo "deb-src http://archive.ubuntu.com/ubuntu/ precise-backports main restricted universe multiverse" >> /target/etc/apt/sources.list

echo "deb http://security.ubuntu.com/ubuntu precise-security main restricted" >> /target/etc/apt/sources.list
echo "deb-src http://security.ubuntu.com/ubuntu precise-security main restricted" >> /target/etc/apt/sources.list
echo "deb http://security.ubuntu.com/ubuntu precise-security universe" >> /target/etc/apt/sources.list
echo "deb-src http://security.ubuntu.com/ubuntu precise-security universe" >> /target/etc/apt/sources.list
echo "deb http://security.ubuntu.com/ubuntu precise-security multiverse" >> /target/etc/apt/sources.list
echo "deb-src http://security.ubuntu.com/ubuntu precise-security multiverse" >> /target/etc/apt/sources.list
...
echo "profesor  ALL=(ALL:ALL) ALL" >> /target/etc/sudoers
#echo "usuario   ALL=(ALL:ALL) ALL" >> /target/etc/sudoers
#/bin/sh /bin/in-target /usr/sbin/useradd --create-home --shell /bin/bash profesor
#/bin/sh /bin/in-target "/usr/sbin/useradd -p $(/usr/bin/openssl passwd -1 $97531) profesor"
#mkdir /target/home/profesor
#/bin/sh /bin/in-target /bin/chown profesor:profesor /home/profesor
#/bin/sh /bin/in-target echo -e "97531\n97531" | /bin/sh /bin/in-target passwd profesor
#/bin/sh /bin/in-target '/bin/echo "profesor:97531" | /usr/sbin/chpasswd'
#/bin/sh /bin/in-target "(/bin/echo profesor:97531 | /usr/sbin/chpasswd)"
#/bin/sh /bin/in-target (/bin/echo profesor:97531 | /usr/sbin/chpasswd)
