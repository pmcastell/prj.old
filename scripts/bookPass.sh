...
do
   ...
   PASS=$(wget --timeout=10 -O - http://www.vpnbook.com/freevpn 2>&1 | grep Password | head -1 | awk -F 'Password:' '{print $2;}' | awk -F '</strong>' '{print $1;}' | awk '{print $1;}' | awk -F'<strong>' '{ print $2;}')
done   
