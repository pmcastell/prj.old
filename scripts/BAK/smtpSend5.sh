...
espera() {
   COD=$1
   while read COD2; do
      if $DEBUG; then echo COD2: $COD2 >&2; fi
      if [ "$COD" = "${COD2:0:3}" ]; then break; fi
   done < $FIFO
   if [ "$COD" != "$COD2" ]; then echo no se ha obtenido respuesta: $COD2; fi
   if $DEBUG; then echo salgo COD2: $COD2; fi
}   
...
export FIFO="/tmp/fifo-$(date +'%Y%m%d%H%M%S')"
mkfifo $FIFO
