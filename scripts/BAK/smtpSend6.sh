...
espera() {
   ...
   while true; do
      ...
      if [ "$COD" = "$COD2" ]; then if $DEBUG; then infor espera: intento salir de bucle; fi ; break; fi
   done
   if $DEBUG; then infor COD: "|$COD|" "-" COD2: "|$COD2|" ; fi
}   
...
envia "."; espera 250
envia "quit"; espera 221 ) | $PROXY_CMD openssl s_client -starttls smtp -connect $SMTP_SERVER -crlf -ign_eof  &> $TEMP
#rm $TEMP
