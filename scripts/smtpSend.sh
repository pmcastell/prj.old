...
main() {
    [ $# -lt 4 ] && uso
    DIR_BASE=$(dirname $0); REMITENTE="$1"; RECEPTOR="$2"; SUBJECT="$3"; TEXTO="$4"
    ...
    if [ "$(wget --timeout=10 --tries=3 -O - http://portquiz.net:$(echo $SMTP_SERVER | awk -F':' '{print $2;}') 2> /dev/null)" = "" ]; then
       echo Usamos tor ya que el puerto $SMTP_SERVER está cerrado
       PROXY_CMD="torsocks "
       for((i=1;i<=5;i++));do 
       ...
       echo Enviamos el correo directamente
