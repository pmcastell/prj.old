...
echo wget -O - "https://www.duckdns.org/update?domains=paquita58&token=${TOKEN}&ip=${IP}&verbose=true" 2>/dev/null
wget -O - "https://www.duckdns.org/update?domains=paquita58&token=${TOKEN}&ip=${IP}&verbose=true" 2>/dev/null
