...
if [ "$(ifconfig | grep -E ^kvmbr0)" = "" ]; then
   ...
   sudo dnsmasq --interface=kvmbr0 --bind-interfaces --dhcp-range=$RED.15,$RED.200,$MASK
...
sudo kvm -machine q35 -soundhw hda -vga qxl -k es -boot menu=on -localtime -net nic,vlan=0,macaddr=$(randomMAC) -net tap,vlan=0 "$@" &
sleep 2
###2
####_remmina $VNCPORT
