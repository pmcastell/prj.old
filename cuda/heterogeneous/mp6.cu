...
__global__ void convolution2DImage(float *N, const float  *M, float *P, int Width, int Height) {
	...
	int tx=threadIdx.x, ty=threadIdx.y, totalWidth=CHANNELS*Width;

	int indx=blockIdx.x*blockDim.x+tx,
		indy=blockIdx.y*blockDim.y+ty,
		haloIndX,haloIndY;
	...
	if (tx>=TILE_SIZE-Mask_radius) { // left
		...
		for(int k=0;k<CHANNELS;k++) {
			N_ds[ty+Mask_radius][haloIndX][k]=(indx-TILE_SIZE<0 || indy>=Height ? 0 : _N(indy,indx-TILE_SIZE,k));
		}
	}
	if (ty>=TILE_SIZE-Mask_radius) { // up
		haloIndY=ty-(TILE_SIZE-Mask_radius);
		for(int k=0;k<CHANNELS;k++) {
			N_ds[haloIndY][tx+Mask_radius][k]=(indy-TILE_SIZE<0 || indx>=Width ? 0 : _N(indy-TILE_SIZE,indx,k));
		}
	}
	if (tx<Mask_radius) { // right
		haloIndX=Mask_radius+TILE_SIZE+tx;
		for(int k=0;k<CHANNELS;k++) {
			N_ds[ty+Mask_radius][haloIndX][k]=(indx+TILE_SIZE>=Width || indy>=Height ? 0 : _N(indy,indx+TILE_SIZE,k));
		}
	}
	if (ty<Mask_radius) { // bottom
		haloIndY=Mask_radius+TILE_SIZE+ty;
		for(int k=0;k<CHANNELS;k++) {
			N_ds[haloIndY][tx+Mask_radius][k]=(indy+TILE_SIZE>=Height || indx>=Width ? 0 : _N(indy+TILE_SIZE,indx,k));
		}
	}
	...
	} else {//	if (indx>=Width || indy>=Height) {
		for(int k=0;k<CHANNELS;k++) {
			N_ds[ty+Mask_radius][tx+Mask_radius][k]=0;
		}
	}
