...
score = 0; lives = 3
time = 0.5; started = False
#globals for the game
rock_group=set([]); missile_group=set([])
explosion_group=set([])
...
def rock_spawner():
    ...
    if (started and len(rock_group)<12):
        ...
        rock_vel = [rock_vel[0]*(1+increment/10),rock_vel[1]*(1+increment/10)]
        rock_avel = random.random() * .2 - .1
        if (dist(my_ship.get_position(),rock_pos)>=5*my_ship.get_radius()):
