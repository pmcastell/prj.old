...
public class superCanvas extends Canvas {
    ...
    int angulo=0, velocidad=0, posInicial=0;
    ...
    public void setPosInicial(int p) {
      this.posInicial=p;
    }
...
//       marcaYizda(redondeo(ymax,numDecimales));
    ...
    void ejes()
    {
       ...
       clear();
       this.establece_rangos(-ox, ox, -oy, oy);
       ejes(-ox, -oy, ox, oy, ox, oy);
//       this.parabolaParam();
//       dibujaParabola();
    }
    ...
    void parabolico() {
       double t=0;
       double x1,x2,y1,y2;
       double tImpacto=Double.parseDouble(controlInfo.getText());
       g=this.getGraphics();
       double x0=this.posInicial;
       double y0=0.0;
       double a1x,a1y,v2x,v2y;
       ox=oy=-x0;

       int f=10;
       this.establece_rangos(-f*ox, f*ox , -f*oy, f*oy);
       ejes(-f*ox, -f*oy, f*ox, f*oy, f*ox, f*oy);

       long time;
       double incremento;
       incremento=0.2;
       double G=1, M=1;
       double v1x=velocidad*Math.cos(angulo*Math.PI/180);
       double v1y=velocidad*Math.sin(angulo*Math.PI/180);
...
//          while (System.currentTimeMillis()<time+incremento);
          a1x=-G*M*x1/Math.pow(Math.pow(x1,2)+Math.pow(y1,2),3/2);
          a1y=-G*M*y1/Math.pow(Math.pow(x1,2)+Math.pow(y1,2),3/2);
          v2x=v1x+a1x*incremento;
          v2y=v1y+a1y*incremento;
          x2=x1+v1x*incremento;
          y2=y1+v1y*incremento;
